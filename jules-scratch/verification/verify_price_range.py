...
def run(playwright):
    ...
    expect(card_table).to_be_visible()

    # Locate the row for "Crydit Card"
    crydit_row = page.locator("tr:has-text('Crydit Card')")

    # Verify the virtual card opening fee displays the correct range
    virtual_card_cell = crydit_row.locator("td").nth(2)
    expect(virtual_card_cell).to_contain_text("$179 - $199")

    # Verify the physical card opening fee displays the correct range
    physical_card_cell = crydit_row.locator("td").nth(3)
    expect(physical_card_cell).to_contain_text("$179 - $199")
