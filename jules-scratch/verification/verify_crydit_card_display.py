...
def run(playwright):
    ...
    expect(card_table).to_be_visible(timeout=15000)

    # Locate the row for "Crydit Card"
    crydit_row = page.locator("tr:has-text('Crydit Card')")

    # --- Verification for Crydit Card ---
    # Assert that the Virtual Card column shows "Not Supported"
    virtual_card_cell = crydit_row.locator("td").nth(2)
    expect(virtual_card_cell).to_contain_text("不支持")

    # Assert that the Physical Card column shows the correct network-specific fees
