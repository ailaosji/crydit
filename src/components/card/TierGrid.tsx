  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {tiers.map((tier, index) => {
        const openingFee =
          tier.fees?.virtualCardPrice ?? tier.fees?.physicalCardPrice ?? tier.fees?.openingFee;
        const isOpeningFeeFree = openingFee === undefined || openingFee === null || openingFee === 0;
                ...
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <span
                    className={`font-semibold ${
                      isOpeningFeeFree ? 'text-green-600' : 'text-gray-900'
                    }`}
                  >
                    {isOpeningFeeFree ? '免费' : `$${openingFee}`}
                  </span>
                </div>
